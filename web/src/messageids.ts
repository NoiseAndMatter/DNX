/**
 * Message ids for every conversation this page has with an instrument — **one allocator, shared**.
 *
...
/** How many ids this page has to work with, once Transfer's range is left alone. */
export const MESSAGE_ID_SPAN = MAX_MESSAGE_ID - FIRST_MESSAGE_ID + 1;

let next = FIRST_MESSAGE_ID;

/**
 * Reserve a run of ids no other conversation will use.
 *
 * `count` is how many requests the conversation will send — **one per chunk** for a file read, so
 * be generous rather than exact. Over-reserving costs only space that wraps around anyway;
...
export function reserveMessageIds(count: number): number {
  ...
  // Wrapped whole rather than split across the top: a reservation straddling the end would hand back
  // a base whose run leaves the u16, and `encodeMessage` would throw mid-conversation with a handle
  // open on the device.
  if (next + count > MAX_MESSAGE_ID) next = FIRST_MESSAGE_ID;
...
/** Reset the allocator. **Tests only** — a page has one conversation space and never restarts it. */
export function resetMessageIdsForTest(): void {
  next = FIRST_MESSAGE_ID;
}

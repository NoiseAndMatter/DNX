...
/** The core package, which is where the formats and the device workflows live. */
...
test("no manifest field is invented", () => {
  ...
  const backup = readFileSync(join(CORE, "device", "backup.ts"), "utf8");
...
test("a +Drive directory DNX cannot read is named, not passed over", () => {
  ...
  const manifest = readFileSync(join(CORE, "project", "dnxfile.ts"), "utf8");

...
/**
 * What writing to a +Drive needs from its host.
 *
 * `ids` is the host's one numbering space, for the same reason backups take it: an allocator per
 * caller is the collision this codebase has paid for twice.
 *
 * The other two are consent, and they are separate on purpose. `confirm` is the review a person
 * reads and agrees to, which is a dialog and therefore the host's. **`gate` is the switch**, the
 * one `writeenable.ts` arms, and it is a required function rather than an assumed check because
 * core has no idea whether a host even has such a switch. It is called before a byte is sent, so a
 * control the page forgot to gate still cannot write, which is the property `writeenable.test.ts`
 * exists to hold.
 */
export interface DriveWriteHost {
  ids: MessageIds;
  gate: () => void;
 ...
 */
export async function entryForSlot(device: ConnectedDevice, ids: MessageIds, slot: number): Promise<Entry> {
  ...
  const entry = wholeListing(reply, "/projects").entries.find((e) => e.index === slot);
  if (!entry) {
...
export async function writeProjectToDrive(
  ...
  const { device, host, slot, payload, image, name } = options;
  ...
  const result = await safeWriteFile({
    ...
    verifyMsgId: host.ids.reserve(IDS_FOR.wholeProject),
  ...
  const back = await readStoredForm(device, host.ids, slot, (done) =>
 ...
 */
export async function projectSlotEntries(device: ConnectedDevice, ids: MessageIds): Promise<Entry[]> {
  const transport = device.api;
  const id = ids.reserve(IDS_FOR.oneMessage);
  const reply = await transport.request(listRequest(id, "/projects"), id, 10_000);
...
export async function emptyProjectSlots(device: ConnectedDevice, ids: MessageIds): Promise<number[]> {
  const transport = device.api;
  const id = ids.reserve(IDS_FOR.oneMessage);
  const reply = await transport.request(listRequest(id, "/projects"), id, 10_000);

 ...
 * arithmetic.
 *
 * Both tables were derived the same way: nine DN1 projects sit beside Elektron's own DN2
 * conversions of them, and the DN1 side is fully decoded, so each DN1 value can be read off
 * against the DN2 value the importer produced at the same table position and step. Every
...
/**
 * Parameter-lock ids. Derived from 392 lock records across nine matched projects.
 *
...
/** Derived from 1,246 conditioned trigs across nine matched projects. */
const CONDITION_MAP = new Map<number, TranslatedCondition>([
  ...
  // The main condition family: FILL, PRE, NEI, 1st, and the A:B ratios.
 ...
 *
 * Unobserved values sit in the gaps of otherwise regular runs (DN1 14, 16..20, 39, 41), so
 * interpolating is tempting. Do not: the observed steps are uneven — 6..13 advance by
 * roughly 8 percentage points but 13 -> 15 jumps 12, and the condition codes step by 1 up to
 * 29 then by 2 — so an interpolated value would be a guess dressed up as arithmetic.
 */

...
| 2b | Split the rest of `probe/main.ts`. **2026-08-01: `cards.ts` (all rendering, including the capture) and `ports.ts` (selection) extracted** — 2,625 → 2,412 lines, and everything moved so far is presentation or selection, which cannot break a device. **What remains is the transports** — the dump I/O and the +Drive I/O — and those want a hardware pass with the change | **high** for the transports; everything done so far was low |

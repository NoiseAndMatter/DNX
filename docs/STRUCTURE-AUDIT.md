...
| 2b | Split the rest of `probe/main.ts`. **2026-08-01: `cards.ts` and `ports.ts` extracted** — presentation and port selection, neither of which can break a device. `capture`/`dumpio`/`storageio` remain, and those are the transports | **high** for what remains; the two done were low |
